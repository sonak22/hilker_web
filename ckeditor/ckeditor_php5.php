...
class CKEditor
{
	...
	public function editor($name, $value = "", $config = array(), $events = array())
	{
		...
		$out = "<textarea id='content' name=\"" . $name . "\"" . $attr . ">"  . htmlspecialchars($value) . "</textarea>\n";
		if (!$this->initialized) {
			$out .= $this->init();
		}
		...
		if($config['toolbar'] == ""){
			$config['toolbar']	= array(
				array('TextColor','BGColor','Source',   'Preview', 'Print', 'Bold','Italic','Underline','Strike','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock','Link','Unlink','HorizontalRule','Image','Flash','Table','Youtube','Font','FontSize'  ),
					'/',
				);
		}
